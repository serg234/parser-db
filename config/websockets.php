...
return [
    ...
    */
    ...
    'apps' => [
        [
            // уникальный ID приложения — должен совпадать с PUSHER_APP_ID из .env
            'id' => env('PUSHER_APP_ID'),

            // произвольное имя (чаще всего APP_NAME)
            'name' => env('APP_NAME'),

            // ключ и секрет из .env
            'key' => env('PUSHER_APP_KEY'),
            'secret' => env('PUSHER_APP_SECRET'),

            // количество одновременных соединений (null = неограниченно)
            'capacity' => null,

            // клиентские события (в вашем случае не нужны)
            'enable_client_messages' => false,

            // статистика (если не нужна — false)
            'enable_statistics' => true,

            // разрешённые origin-запросы из браузера
            'allowed_origins' => [
                env('APP_URL'),      // например, http://start.local
                // или просто ['*'] для локальной разработки
            ],
        ],
    ],
